...
The trend in N$_2$ and N$_2$H binding observed shows a moderate correlation with the cohesive energies of the substituent metals as seen in Figure \ref{fig:cohesive_energies}. This indicates that the binding between the

\begin{equation}
    E_{coh} = \epsilon_d + \epsilon_s \\
    E_{coh} = \int_{B_d}^{E_F} (E_d-E)\rho(E)dE
\end{equation}

where $E_{coh}$ is the cohesive energy, $B_d$ is the bottom of the d band, $E_F$ is the fermi level energy
